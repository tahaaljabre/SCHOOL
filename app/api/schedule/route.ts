...
export async function GET(){try{
 ...
 const filter=auth.isGlobal?"":" WHERE c.shift_id=?",bind=auth.isGlobal?[]:[auth.activeShiftId];
 const [entries,classes,teachers,subjects,settings,shifts]=await Promise.all([
  auth.db.prepare(`SELECT e.*,c.name classroom_name,t.full_name teacher_name,s.name subject_name,c.shift_id FROM schedule_entries e JOIN classrooms c ON c.id=e.classroom_id JOIN teachers t ON t.id=e.teacher_id JOIN subjects s ON s.id=e.subject_id${filter} ORDER BY e.day_of_week,e.period_number`).bind(...bind).all(),
  auth.db.prepare(`SELECT id,name,shift_id FROM classrooms WHERE archived_at IS NULL${auth.isGlobal?"":" AND shift_id=?"}`).bind(...bind).all(),
  auth.db.prepare(`SELECT DISTINCT t.id,t.full_name name FROM teachers t JOIN teacher_shifts ts ON ts.teacher_id=t.id WHERE t.archived_at IS NULL${auth.isGlobal?"":" AND ts.shift_id=?"}`).bind(...bind).all(),
  auth.db.prepare("SELECT id,name FROM subjects WHERE archived_at IS NULL ORDER BY name").all(),auth.db.prepare("SELECT work_days FROM school_settings WHERE id=1").first<{work_days:string}>(),auth.db.prepare("SELECT id,name,starts_at,ends_at,gender_scope FROM shifts WHERE status='ACTIVE' ORDER BY id").all()
 ]);
 return Response.json({entries:entries.results,classes:classes.results,teachers:teachers.results,subjects:subjects.results,workDays:JSON.parse(settings?.work_days||"[6,0,1,2,3]"),shifts:shifts.results,activeShiftId:auth.activeShiftId});
