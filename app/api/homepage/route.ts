...
export async function PUT(request:Request){
 try{const auth=await requireSchoolAdmin();if(auth.error)return auth.error;const p=await request.json() as Record<string,unknown>,action=text(p.action,30);
  if(action==="settings"){
   ...
   let imageUrl=text(p.imageUrl,1000),mediaUrl=text(p.mediaUrl,1000);if(!mediaUrl&&/(?:youtu\.be|youtube\.com)/i.test(imageUrl)){mediaUrl=imageUrl;imageUrl="";}const values=[itemType,title,text(p.body,1200),imageUrl,mediaUrl,Number(p.sortOrder)||0,Boolean(p.published)?1:0];
   if(action==="create")await auth.db.prepare("INSERT INTO public_home_items(item_type,title,body,image_url,media_url,sort_order,published) VALUES(?,?,?,?,?,?,?)").bind(...values).run();
   else {const id=Number(p.id);if(!id)return Response.json({error:"البطاقة غير موجودة"},{status:400});await auth.db.prepare("UPDATE public_home_items SET item_type=?,title=?,body=?,image_url=?,media_url=?,sort_order=?,published=?,updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(...values,id).run();}
