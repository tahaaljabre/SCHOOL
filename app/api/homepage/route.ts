...
export async function PUT(request:Request){
 try{const auth=await requireSchoolAdmin();if(auth.error)return auth.error;const p=await request.json() as Record<string,unknown>,action=text(p.action,30);
  if(action==="settings"){
   ...
   let imageUrl=text(p.imageUrl,4000),mediaUrl=text(p.mediaUrl,1000);if(!mediaUrl&&/(?:youtu\.be|youtube\.com)/i.test(imageUrl)){mediaUrl=imageUrl;imageUrl="";}const group=text(p.achieverGroup,20)==="EVENING"?"EVENING":"MORNING",values=[itemType,title,text(p.body,2400),imageUrl,mediaUrl,text(p.achieverPercentage,20),text(p.achieverAverage,20),text(p.achieverRank,80),group,itemType==="ACHIEVER"&&Boolean(p.honorFeatured)?1:0,Number(p.sortOrder)||0,Boolean(p.published)?1:0];
   if(action==="create")await auth.db.prepare("INSERT INTO public_home_items(item_type,title,body,image_url,media_url,achiever_percentage,achiever_average,achiever_rank,achiever_group,honor_featured,sort_order,published) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)").bind(...values).run();
   else {const id=Number(p.id);if(!id)return Response.json({error:"البطاقة غير موجودة"},{status:400});await auth.db.prepare("UPDATE public_home_items SET item_type=?,title=?,body=?,image_url=?,media_url=?,achiever_percentage=?,achiever_average=?,achiever_rank=?,achiever_group=?,honor_featured=?,sort_order=?,published=?,updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(...values,id).run();}
